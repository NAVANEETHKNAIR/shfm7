    ...
    { 
           ...
           $qry="SELECT * FROM users WHERE mob_no = '".$mob."' ";
            ...
            while($row=mysql_fetch_array($sqll))
            {
              ...
              $sq="SELECT * FROM exception WHERE status = '0' and dev_id='$dev'";
              $ss=mysql_query($sq);
              while($row1=mysql_fetch_array($ss))
              {
                $sense=$row1["sensor"];
                $val=$row1["value"];
                if($sense="TempSensorexception")
                {
                  echo "Temperature  :" .$val;
                }
                else if($sense="moistureexception")
                {
                  echo "Moisture   :" .$val;
                }
                else if($sense="pHexception")
                {
                  echo "pH  :" .$val;
                }
            ...
            $up="UPDATE exception SET status='1' WHERE dev_id='$dev'";
                $sq=mysql_query($up);
