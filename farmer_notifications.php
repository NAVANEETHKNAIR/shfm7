		       ...
		       $qry="SELECT * FROM users WHERE mob_no='$mob' limit 1" ; 
				$sql=mysql_query($qry);	
					...
					while($row=mysql_fetch_array($sql))
					  		{
					  			$idd=$row["id"];
					  			$pamuco_nm=$row["pamuco_name"];
					  			$qr="SELECT * FROM users WHERE pamuco_name='$pamuco_nm'AND category='farmer' " ; 
								$sq=mysql_query($qr);	

								while($row=mysql_fetch_array($sq))
								{
									$fid=$row["id"];
									$q="SELECT * FROM enquiry WHERE uid='$fid'" ; 
									$s=mysql_query($q);
									while($row=mysql_fetch_array($s))
									{
										$qid=$row["id"];
					  					//echo $row["question"]."<a href=>Reply</a>";
					  					//echo "<br>";
                    $ans=$row["answer"];
                    if($ans)
                    {
                    ...
                    else
                    {
                      ...
                      echo ".";
					  					//echo '&nbsp';
  //echo '&nbsp';echo '&nbsp';
					  					//echo $row["question"].'<a href="reply.php?id='.$qid.'" target="popup" onclick="window.open(http://localhost/reply.php?id='.$qid.',"_self"); return false;">Reply</a>';
					  					//echo "<br>";
...
/*echo $row["question"];
echo "<a  href=reply.php?id=$qid>";
echo "<a href=reply.php?id=$id>";
echo '<div class="rep">';
   echo '<img src="assets/img/rep.png" width="30" height="30" title="reply" >';
   echo '</div>';
   echo '</a>';
